...
class TestSearchDoctorListSdk(unittest.TestCase):
  ...
  def test_yishengliebiao(self): # 1.医生列表页老接口
  ...
  def test_nankezhongxin(self): # 2.男科中心落地页
  ...
  def test_sanwensandayizheng(self):  # 3.三问三答义诊
  ...
  def test_dingxiangIMhuyiqingzixun(self):  #  4.定向IM/互医轻咨询（老）
  ...
  def test_dingxiangyisheng(self): # 5. 5041-定向医生
    result = temp["result"]
  ...
  def test_wenyishengka(self): # 6.5192-问医生卡
  ...
  def test_jingxuanyishengzhuanqu(self):# 7.精选医生专区
  ...
  def test_guahaopinpaika(self):#8. 5589-挂号品牌卡
  ...
  def test_qiangfenzhiyiliuliangwenzhengchonggou(self):
    # 9.千分之一流量-问诊重构
  ...
  def test_nankepingpaika(self):  #10.50275-男科品牌卡
  ...
  def test_CMS_yemian(self):
      print("11.cms页面")

  @pytest.mark.run(order=12)
  def test_zijianye_qianruyishengliebiaoiframe(self): #12.自建页-嵌入医生列表iframe
  ...
  def test_yiyuantiaozhuanyishengluodiye(self):   #13.医院跳转医生落地页
